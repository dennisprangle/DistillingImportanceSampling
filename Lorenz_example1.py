...
while dis.eps > 0. or dis.ess < 250.:
    ...
    pars_sample = samp[:,0:4]
    df = pd.DataFrame(
        pars_sample.detach().numpy(),
    ...
    plt.savefig('Lorenz_ex1_pars.pdf')
    ...
    for j in range(paths_toplot):
        plt.plot(range(T+1), paths[j,0,:], 'r-.', alpha=0.3, label="$x_1$" if j==0 else "")
        plt.plot(range(T+1), paths[j,1,:], 'b:', alpha=0.3, label="$x_2$" if j==0 else "")
        plt.plot(range(T+1), paths[j,2,:], 'g--', alpha=0.3, label="$x_3$" if j==0 else "")
    ...
    plt.xlabel('i')
    plt.savefig('Lorenz_ex1_paths.pdf')

np.save('Lorenz_example1_pars', pars_sample.detach().numpy())
